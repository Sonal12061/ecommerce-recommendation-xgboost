...
│   ├── df_clean.csv               # Cleaned transaction data
│   ├── df_anonymous.csv           # Anonymous rows for popularity fallback
│   ├── popularity.csv             # Global product popularity ranking
│   ├── rfm.csv                    # User-level RFM features
│   ├── products.csv               # Product-level features + category
│   ├── customer_category.csv      # Category affinity per customer
│   ├── customer_products.csv      # Purchase history per (customer, product)
│   ├── train_data.csv             # Final training dataset with labels
│   ├── test_data.csv              # Final test dataset with labels
│   └── test_predictions.csv      # Model predictions for evaluation
│
...
├── 03_Train_Test_Split_          # Time-based split + negative sampling
...
jupyter notebook 03_Train_Test_Split_Negative_Sampling.ipynb

# 4. Model training + explainability
jupyter notebook 04_XGBoost_Training.ipynb
